jQuery(document).ready(function ($) {
  ...
  // Function to fetch and populate product quantities
  function fetchProductQuantities() {
    ...
    jQuery.ajax({
      ...
      type: "POST",
      data: {
        action: "populate_product_quantities",
      ...
      success: function (data) {
        ...
        if (data) {
          jQuery.each(data, function (product_id, quantity) {
            var inputElement = jQuery(
              'input[name="product_quantity[' + product_id + ']"]'
            );
  ...
  // Add an event listener to detect changes in the select input for month
  jQuery(document).on("change", "#month", function () {
    fetchProductQuantities(); // Fetch new data when month changes
    // Update month and year when changing
    var selectedMonth = jQuery("#month").val();
    var selectedYear = jQuery("#year").val();
    jQuery('input[name="selectedYear"]').val(selectedYear);
    jQuery('input[name="selectedMonth"]').val(selectedMonth);
