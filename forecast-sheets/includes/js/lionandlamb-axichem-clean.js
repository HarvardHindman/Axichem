  ...
  function fetchProductQuantities() {
    ...
    jQuery.ajax({
      ...
      success: function (response) {
        ...
        // Check if we have the new response format with product names
        if (data.quantities && data.product_names) {
          console.log('Using new response format with product names');
          productData = data.quantities;
          productNames = data.product_names;
        } else {
          // Old format - just quantities
          console.log('Using old response format (quantities only)');
          productData = data;
          productNames = {};
        }
        ...
        if (productIds.length > 0) {
          ...
          if (missingProductIds.length > 0) {
            ...
            missingProductIds.forEach(function(productId) {
              ...
              if (productNames && productNames[productId]) {
                ...
                // Set the quantities from saved data
                if (productData[productId]) {
                  console.log('Setting quantities for product:', productId);
                  jQuery.each(productData[productId], function(month, quantity) {
                    const inputElement = jQuery(`input[name="product_quantity[${productId}][${month}]"]`);
                    if (inputElement.length) {
                      inputElement.val(quantity);
                ...
                jQuery.ajax({
                  ...
                  success: function(response) {
                    ...
                    if (response.success) {
                      ...
                      // Set the quantities from saved data
                      if (productData[productId]) {
                        console.log('Setting quantities for product after API fetch:', productId);
                        jQuery.each(productData[productId], function(month, quantity) {
                          const inputElement = jQuery(`input[name="product_quantity[${productId}][${month}]"]`);
                          if (inputElement.length) {
                            inputElement.val(quantity);
        ...
        // For each product already in the table, update its quantities
        const tableProductIds = jQuery("#myTable tbody tr[product-id]").map(function() {
          return jQuery(this).attr('product-id');
        }).get();
        console.log('Updating quantities for existing products:', tableProductIds);
        ...
        // Update row totals after populating quantities
